...
export function createRelayE2eeTransportFactory(args: {
  ...
  // Reject malformed offers before opening a socket; never fall back to plaintext.
  importPublicKey(args.daemonPublicKeyB64);
  return (options) => {
    const base = args.baseFactory(options);
...
export function createEncryptedTransport(
  ...
  const startHandshake = async () => {
    if (closed || handshakeStarted) return;
    ...
    try {
      ...
      if (closed) {
        channel.close(1000, "Transport closed");
      ...
      emitClose({ code: 4001, reason: "E2EE handshake failed" });
    }
  };
