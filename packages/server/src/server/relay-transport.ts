...
import { buildRelayWebSocketUrl } from "@omp-desktop/protocol/daemon-endpoints";
...
function closeRelaySocket(
  ...
  close: () => void = () => socket.close(1001, "Relay configuration changed"),
): Promise<void> {
  ...
  return new Promise<void>((resolve, reject) => {
    const timeout = setTimeout(() => {
      try {
        ...
        // A failed hard stop must not permit a competing control registration.
        reject(error);
        return;
      }
      resolve();
    ...
    try {
      ...
      try {
        ...
        // The bounded fallback also handles transports that throw during close.
      }
    }
  });
}
...
export function startRelayTransport({
  ...
  if (!daemonKeyPair) {
    throw new Error("Relay transport requires a daemon E2EE keypair");
  }
  ...
  const dataOpenTimeouts = new Map<RelayWebSocketLike, ReturnType<typeof setTimeout>>();
  // Keep closing sockets owned until their close event, even after a client is
  // removed from the connection-id registry.
  const ownedDataSockets = new Map<RelayWebSocketLike, () => void>();
  ...
  const stop = (): Promise<void> => {
    ...
    for (const timeout of dataOpenTimeouts.values()) clearTimeout(timeout);
    dataOpenTimeouts.clear();
    stopPromise = (async () => {
      await Promise.all(
        [...ownedDataSockets].map(([socket, close]) => closeRelaySocket(socket, close)),
      );
      ownedDataSockets.clear();
      dataSockets.clear();
      if (controlWs) {
        await closeRelaySocket(controlWs);
        controlWs = null;
      }
    })();
    return stopPromise;
  ...
  const connectControl = (): void => {
    ...
    socket.on("open", () => {
      if (stopped || controlWs !== socket) return;
  ...
  const ensureClientDataSocket = (connectionId: string): void => {
    ...
    ownedDataSockets.set(socket, () => socket.close(1001, "Relay configuration changed"));
    ...
    dataOpenTimeouts.set(socket, openTimeout);

    socket.on("open", () => {
      clearTimeout(openTimeout);
      dataOpenTimeouts.delete(socket);
      if (stopped || dataSockets.get(connectionId) !== socket) {
        socket.terminate();
        return;
      }
      ...
      void attachEncryptedSocket(
        ...
        externalMetadata,
        (encryptedSocket) => {
          ownedDataSockets.set(socket, () =>
            encryptedSocket.close(1001, "Relay configuration changed"),
          );
        },
      );
    });
    ...
    socket.on("close", (code, reason) => {
      clearTimeout(openTimeout);
      dataOpenTimeouts.delete(socket);
      ownedDataSockets.delete(socket);
      relayLogger.warn(
        { code, reason: reason?.toString?.(), url, connectionId },
...
async function attachEncryptedSocket(
  ...
  onEncryptedSocket?: (socket: RelaySocketLike) => void,
): Promise<void> {
  try {
    ...
    onEncryptedSocket?.(encryptedSocket);
