...
export function createEncryptedRelaySocket(params: {
  ...
  const close = (code?: number, reason?: string) => {
    ...
    readyState = 2;
    // Encryption and the physical send callback must finish before the close
    // frame. In particular, config responses travel on the retiring relay.
    void (async () => {
  ...
  return {
    ...
    send: (data) => {
      ...
      const pending = channel.send(outbound).catch((error) => {
